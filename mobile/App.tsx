...
export default function App() {
  ...
  const [editTarget, setEditTarget] = useState<'activities' | 'health' | null>(null)
  ...
  return (
    <View style={[styles.appContainer, { backgroundColor: colors.background }]}>
      ...
      {phase === 'main' && editTarget === 'activities' && (
        <OnboardingScreen
          userName={user.name}
          initialActivities={userActivities}
      ...
      {phase === 'main' && editTarget === 'health' && (
        <HealthConditionsScreen
          userName={user.name}
          initialConditions={userHealthConditions}
          onComplete={(selectedConditions) => {
            setUserHealthConditions(selectedConditions)
            setEditTarget(null)
          }}
          onBack={() => setEditTarget(null)}
        ...
        <View style={styles.mainContainer}>
          ...
          {currentTab === 'profile' && (
            <ProfileScreen
              ...
              onEditActivities={() => setEditTarget('activities')}
              onEditHealthConditions={() => setEditTarget('health')}
              onResetToOnboarding={() => {
                setOnboardingStep(1)
                setPhase('onboarding')
              }}
